...
- `apps/editor/src/components/animation` — outil Animation (cadrage `docs/cadrage-animation.md`, D60 : un tiroir sous le canevas, pas un mode ; recentré le 24 septembre 2026 sur « un élément, ses images-clés, son apparence à chaque image-clé »). `SceneEditor` : la scène de la section de l'élément sélectionné (une ligne par élément, une barre par mouvement, les images-clés de l'élément sélectionné, tout se tire), à droite la rubrique `QuickAnimations` (Apparition, Vitesse, Démarre, Délai…), les déclencheurs de survol et de clic (`TriggerSettings`) et l'image-clé à la tête de lecture (`KeyframePanels`, panneaux Design en mode image-clé par `components/design/useKeyframeStyle.ts`). Aides pures : `apps/editor/src/lib/scene-view.ts` (lignes, barres, images-clés en temps de scène), `lib/scene.ts` (la scène d'une section en phrase), `lib/timeline.ts` (règle, libellés, éléments d'une scène), aperçu à la tête de lecture dans `lib/scrub.ts`. Les aides du modèle sont dans `packages/model/src/animations.ts` et `appearance.ts`.
