...
import { animationById, applyOps, describeInteraction, describeTrigger, planRemoveReveal, planReveal, revealOf, sampleSite, toggleInteraction, type Node, type Site } from "../src";
...
describe("apparitions", () => {
  ...
  it("pose une animation d'apparition sans toucher au style, la remplace sans doublon, puis la retire proprement", () => {
    const site = withNode();
    const { site: s1 } = applyOps(site, planReveal(site, node, { kind: "fade-up", delay: 120 }));
    const n1 = s1.pages[0]!.root.children![0]!;
    expect(n1.style?.base).toEqual({ padding: "1rem" });
    expect(revealOf(s1, n1)?.options).toMatchObject({ kind: "fade-up", delay: 120, duration: 700 });
    expect(s1.animations).toHaveLength(sampleSite.animations.length + 1);
    const { site: s2 } = applyOps(s1, planReveal(s1, n1, { kind: "blur" }));
    ...
    const { site: s3 } = applyOps(s2, planRemoveReveal(s2, n2));
  ...
  it("décrit une interaction et un déclencheur en français", () => {
    ...
    const site = withNode();
    const { site: s1 } = applyOps(site, planReveal(site, node, { kind: "zoom" }));
