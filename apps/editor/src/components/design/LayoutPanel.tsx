        ...
        {isGrid ? (
          <>
            {row("gridTemplateColumns", "Colonnes", (
              <div className="flex items-center gap-1 flex-1">
                <div className="flex items-center gap-1 flex-1 min-w-0"><TextInput mono className="flex-1 min-w-0" value={str(s.value("gridTemplateColumns")) ?? ""} placeholder="1fr 1fr" onValueChange={(v) => s.set("gridTemplateColumns", v || undefined)} /><TokenSelect site={site} group="space" onPick={(t) => s.set("gridTemplateColumns", t)} /></div>
            ...
            {row("gridTemplateRows", "Lignes", <div className="flex items-center gap-1 flex-1 min-w-0"><TextInput mono className="flex-1 min-w-0" value={str(s.value("gridTemplateRows")) ?? ""} placeholder="auto" onValueChange={(v) => s.set("gridTemplateRows", v || undefined)} /><TokenSelect site={site} group="space" onPick={(t) => s.set("gridTemplateRows", t)} /></div>)}
      ...
      {parentDisplay === "flex" || parentDisplay === "grid" ? (
        <Section title="Place dans son parent" defaultOpen={false} hint="Le parent de cet élément range ses enfants en ligne, en colonne ou en grille. Ici, vous réglez comment cet élément-ci s'y comporte.">
          {parentDisplay === "flex" ? (
            ...
            <>
              {row("gridColumn", "Colonne", <div className="flex items-center gap-1 flex-1 min-w-0"><TextInput mono className="flex-1 min-w-0" value={str(s.value("gridColumn")) ?? ""} placeholder="auto" onValueChange={(v) => s.set("gridColumn", v || undefined)} /><TokenSelect site={site} onPick={(t) => s.set("gridColumn", t)} /></div>)}
              {row("gridRow", "Ligne", <div className="flex items-center gap-1 flex-1 min-w-0"><TextInput mono className="flex-1 min-w-0" value={str(s.value("gridRow")) ?? ""} placeholder="auto" onValueChange={(v) => s.set("gridRow", v || undefined)} /><TokenSelect site={site} onPick={(t) => s.set("gridRow", t)} /></div>)}
            </>
          )}
