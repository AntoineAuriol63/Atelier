...
export function AnimationsPanel({ site, node, commit, onPlay, onOpenAnimation, onTestOnSite, onSelectNode }: { site: Site; node: Node; commit: Commit; onPlay?: (triggerId: string, hostId?: string) => void; onOpenAnimation?: (triggerId?: string) => void; onTestOnSite?: (nodeId?: string) => void; onSelectNode?: (id: string) => void }) {
  ...
  return (
    <Section title="Animations" defaultOpen={triggers.length > 0 || !!appearanceOf(site, node.id) || !!inheritedAppearance(site, node.id)} hint="Choisissez une apparition, une réaction au survol ou un mouvement continu. Le outil Animation règle les déclencheurs et compose des lignes de temps sur plusieurs éléments.">
          ...
          {triggers.map((t) => {
            ...
            return (
              <li key={t.id} className="flex items-center gap-1 rounded-sm border border-line bg-surface/60 px-2 py-1">
                {onOpenAnimation && a
                  ? <button type="button" className="flex-1 min-w-0 min-h-7 text-left text-xs text-ink truncate rounded-xs hover:text-accent" title={`Modifier dans le outil Animation · ${describeTrigger(t, site)}`} onClick={() => onOpenAnimation(t.id)}>{describeTrigger(t, site)}</button>
