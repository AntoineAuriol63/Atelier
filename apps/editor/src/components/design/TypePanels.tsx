  ...
  return (
    <Section title="Formulaire" hint="Les envois arrivent dans Données → Messages reçus, et par email si la notification est configurée.">
      <FieldGroup>
        ...
        <Field label="Service d'envoi" hint="Vide : les envois arrivent dans Atelier (Messages reçus, email), aussi depuis un site exporté. Une adresse (Formspree, Netlify Forms, votre fonction) : le formulaire y poste directement et ne dépend plus d'Atelier."><TextInput mono value={String(node.props.endpoint ?? "")} placeholder="https://formspree.io/f/…" onValueChange={(v) => commit({ op: "node.set", id: node.id, path: "props.endpoint", value: v.trim() || undefined }, { coalesceKey: `endpoint:${node.id}`, label: "Service d'envoi" })} /></Field>
