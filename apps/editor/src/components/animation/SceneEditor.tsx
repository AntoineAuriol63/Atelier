                ...
                <ul aria-label={`Scène de ${quoteLabel(view.sectionLabel)}`}>
                  ...
                  {row.selected && ap && hoverT !== null && !playing && !drag && !ap.track.keyframes.some((k) => Math.abs(k.at + (ap.trigger.delay ?? 0) - hoverT) < 20) ? (
                    <button type="button" data-scene-ghost="" aria-label="Fixer l'état ici (une image-clé)" title={`Fixe à quoi ressemble l'élément à ${tickLabel(hoverT)} ms (une image-clé)`}
                      className="group absolute top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 flex items-center gap-1 h-5 pl-1 pr-1.5 rounded-full border border-dashed border-accent/80 bg-panel/95 text-2xs text-accent whitespace-nowrap shadow-md backdrop-blur-sm transition-[transform,background-color,color,box-shadow,border-color] duration-150 ease-out motion-reduce:transition-none hover:scale-105 hover:bg-accent hover:text-accent-ink hover:border-solid hover:border-accent hover:shadow-[0_0_0_4px_var(--color-accent-soft),0_6px_16px_rgba(0,0,0,.35)] active:scale-95" style={{ left: pct(hoverT) }}
                      onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); const at2 = Math.max(0, hoverT - (ap.trigger.delay ?? 0)); run(planSetKeyframe(getSite(), ap.animation.id, ap.track.id, at2, {}), `État fixé à ${at2} ms`); place(hoverT); }}><Diamond size={9} className="shrink-0 transition-transform duration-150 group-hover:rotate-90 group-hover:scale-110" aria-hidden /><span>Fixer l&apos;état ici</span><span className="text-[10px] tabular-nums opacity-70 group-hover:opacity-100">{tickLabel(hoverT)} ms</span></button>
                  ) : null}
