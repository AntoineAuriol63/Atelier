...
import { ANIMATION_PRESETS, TRIGGER_LABELS, animationUsages, applyOps, indexSite, keyframeAt, newId, planAddTrigger, planAppearanceStart, planQuickAnimation, planSetKeyframe, planSetKeyframeEasing, trackPresetMatch } from "@atelier/model";
            ...
            {view.rows.map((row, i) => (
              <li key={row.id} data-scene-row={row.id} data-still={row.still || undefined} className={`grid grid-cols-[220px_minmax(0,1fr)] items-center h-8 border-b border-line/60 ${row.selected ? "bg-accent-soft/40" : ""}`}>
                ...
                <div className="relative h-8 min-w-0">
                  {row.bar ? (
                    <div data-scene-bar="" className={`absolute top-1.5 h-5 rounded-sm border text-2xs leading-none flex items-center px-1.5 overflow-hidden whitespace-nowrap ${row.selected ? "bg-accent text-accent-ink border-accent" : "bg-accent/25 border-accent/50 text-ink"}`} style={{ left: pct(row.bar.start), width: `max(6px, calc(${pct(row.bar.end)} - ${pct(row.bar.start)}))` }} title={`${tickLabel(row.bar.start)} → ${tickLabel(row.bar.end)} ms`}>
                      {row.keyframes ? null : presetLabel(row.bar.preset) ?? "Mouvement"}
                    </div>
                  ) : row.withGroup ? (
                  ...
                  {row.keyframes?.map((k) => (
                    <button key={k.at} type="button" data-scene-kf={k.at} aria-label={`Image-clé à ${tickLabel(k.sceneAt)} ms`} aria-pressed={at === k.at} title={`${k.sceneAt} ms${k.easing ? ` · ${k.easing}` : ""}`}
                      className={`absolute top-1/2 -translate-x-1/2 -translate-y-1/2 p-1 rounded-xs ${at === k.at ? "text-warning" : "text-accent-ink hover:text-warning"}`} style={{ left: pct(k.sceneAt) }} onClick={() => place(k.sceneAt)}>
