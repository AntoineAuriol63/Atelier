...
import {
  ANIMATION_PRESETS, TRIGGER_LABELS, animationLength, appearanceAnchors, appearanceOf, indexSite, planAppearanceDelay, planAppearanceDetail, planAppearancePreset,
...
import { Button, Field, FieldGroup, IconButton, NumberInput, Select, Toggle } from "@/ui";
 ...
 */
export function QuickAnimations({ site, node, commit, onOpenAnimation, onPlay, onTestOnSite }: { site: Site; node: Node; commit: Commit; onOpenAnimation?: () => void; onPlay?: PlayFn; onTestOnSite?: () => void }) {
  ...
  return (
    <FieldGroup>
      <AppearanceFields site={site} node={node} run={run} onPlay={onPlay} />
...
const beginValue = (b: AppearanceBegin) => (b.kind === "own" ? `own:${b.on}` : b.kind === "host" ? "host" : `${b.kind}:${b.node}`);

/** L'apparition de l'élément : effet, vitesse, démarrage, délai, rejouer, phrase de résumé et détail (lettres, enfants). */
function AppearanceFields({ site, node, run, onPlay }: { site: Site; node: Node; run: Run; onPlay?: PlayFn }) {
  const ap = appearanceOf(site, node.id);
  ...
  const beginOptions = ap ? [
    ...
    ...(ap.begin.kind === "host" ? [{ value: "host", label: ap.page ? `avec la page (${TRIGGER_LABELS[ap.trigger.on].toLowerCase()})` : `avec ${labelOf(ap.hostId)} (${TRIGGER_LABELS[ap.trigger.on].toLowerCase()})` }] : []),
  ...
  const setBegin = (v: string) => {
    ...
    const begin: AppearanceBegin = kind === "own" ? { kind: "own", on: rest as "load" | "inView" } : { kind: kind as "after" | "with", node: rest };
