...
export function TreeRow({ id, depth, label, meta, icon: Icon, selected, open, hasChildren, onToggle, onSelect, dimmed, editing, onRename, onEditStart, drop, refusal, draggable, trailing, onDragStart, onDragOver, onDragLeave, onDrop }: {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={cx(
        "relative group flex items-center h-8 pr-2 text-sm border-l-2 select-none cursor-default focus-visible:outline-2 focus-visible:outline-accent focus-visible:-outline-offset-2",
    ...
    >
      ...
      <button
        ...
        className={cx("w-5 h-5 mr-1 inline-flex items-center justify-center rounded-xs text-dim hover:text-ink", !hasChildren && "invisible")}
        aria-label={open ? "Replier" : "Déplier"}
      >
        <ChevronRight size={13} className={cx("transition-transform", open && "rotate-90")} aria-hidden />
      </button>
      <Icon size={14} strokeWidth={1.75} className={cx("mr-1.5 shrink-0", selected ? "text-accent" : "text-muted")} aria-hidden />
      {editing ? (
        <input
          autoFocus
          value={draft}
