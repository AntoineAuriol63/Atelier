...
export function Segmented({ value, options, onChange, className, size = "md" }: { value: string | undefined; options: SegmentOption[]; onChange: (v: string | undefined) => void; className?: string; size?: "sm" | "md" }) {
  return (
    <div role="radiogroup" className={cx("inline-flex items-stretch rounded-sm bg-surface border border-line p-0.5 gap-0.5", className)}>
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className={cx("flex-1 inline-flex items-center justify-center rounded-xs text-xs whitespace-nowrap", size === "sm" ? "h-5 min-w-5 px-1" : "h-6 min-w-6 px-1.5", active ? "bg-raised text-ink shadow-sm" : "text-muted hover:text-ink hover:bg-hover")}
          >
