...
export function Tabs({ tabs, value, onChange, variant = "underline", className }: { tabs: Tab[]; value: string; onChange: (id: string) => void; variant?: "underline" | "pill"; className?: string }) {
  return (
    <div role="tablist" className={cx("flex items-stretch min-w-0", variant === "pill" ? "gap-0.5 rounded-sm bg-surface p-0.5" : "border-b border-line overflow-hidden", className)}>
      {tabs.map((t) => {
