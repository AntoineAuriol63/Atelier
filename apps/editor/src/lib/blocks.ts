...
export type BlockPreset = { id: string; label: string; description: string; icon: LucideIcon; group: "Sections" | "Structure" | "Contenu" | "Données" | "Composants"; make: (site: Site) => Node };
...
const section = (site: Site, name: string, children: Node[], base: Record<string, unknown> = {}): Node => ({ id: newId(), type: "box", name, props: { tag: "section" }, style: { shared: shared(site, "Section"), base: { display: "flex", flexDirection: "column", gap: { token: "space.5" }, maxWidth: { token: "width.content" }, marginLeft: "auto", marginRight: "auto", ...base } }, children });
...
export const SECTIONS: BlockPreset[] = [
  { id: "sec-hero", label: "Héros", description: "Surtitre, grand titre, texte et deux boutons, avec une image", icon: LayoutGrid, group: "Sections", make: (site) => section(site, "Héros", [
    ...
    image("4 / 5"),
  ], { display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: { token: "space.10" }, alignItems: "center" }) },
  { id: "sec-text-image", label: "Texte et image", description: "Un titre et un paragraphe à côté d'une image", icon: Columns2, group: "Sections", make: (site) => section(site, "Texte et image", [
    ...
    image("4 / 3"),
  ], { display: "grid", gridTemplateColumns: "1fr 1fr", gap: { token: "space.8" }, alignItems: "center" }) },
  { id: "sec-features", label: "Trois points forts", description: "Trois colonnes avec titre et texte", icon: Columns2, group: "Sections", make: (site) => section(site, "Points forts", [
  ...
  { id: "sec-cta", label: "Appel à l'action", description: "Un titre, une phrase et un bouton, sur fond de surface", icon: MousePointerClick, group: "Sections", make: (site) => section(site, "Appel à l'action", [
  ...
  { id: "sec-faq", label: "Questions fréquentes", description: "Trois questions et leurs réponses", icon: List, group: "Sections", make: (site) => section(site, "Questions fréquentes", [
