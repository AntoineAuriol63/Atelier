...
12. Minimum professionnel : code head réel, 404 personnalisée, redirections, nom de site modifiable, destinataire par formulaire. **Fait le 9 septembre 2026** : site publié servi comme document HTML complet par la fonction de l'export (code head et fin de body tels quels, plus de page React), page `/404`, redirections avec préfixe, nom du site, destinataire par formulaire ; le tout dans la fenêtre Publier et le panneau Formulaire.
13. Rôles et partage par site, publication du contenu seule ; aperçu et tableur navigables au clavier. **Fait le 9 septembre 2026** : rôles propriétaire / éditeur / rédacteur (frontière Écriture/Design appliquée aux opérations côté serveur), table `site_members`, invitations dans Publier → Partage, connexion des invités, « Publier les contenus seulement », flèches et Entrée dans le tableur. L'aperçu se parcourt déjà aux flèches depuis les calques et le mode Écriture ; le parcours des calques au clavier complet et les rôles ARIA restent notés.
...
15. Le deuxième site d'une autre nature avant toute nouvelle fonctionnalité. **Fait le 9 septembre 2026, en code puis refait depuis l'éditeur (`docs/audit-usage-restaurant.md`, onze points relevés, à traiter avant toute nouvelle fonctionnalité)** : « Maison Aurèle », restaurant (carte et événements en bases, page par événement, réservation, composants avec propriétés et variantes, apparitions, questions dépliables, ancres, redirections, 404), gabarit à la création. A demandé la première tranche des interactions déclaratives (D31), qui manquaient au modèle rendu. **Reprise le 10 septembre 2026** : les onze blocages relevés corrigés dans l'éditeur, site d'audit remis au niveau du site codé depuis l'interface seule (§ 18 de l'audit), et l'exploration parallaxe / bandeau / carrousel automatique / compteurs livrée.
