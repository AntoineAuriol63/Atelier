...
1. Le navigateur poste sur `POST /api/forms/<formId>` (multipart). Avec le script du site, l'envoi se fait sans rechargement et le message de succès ou d'erreur apparaît en place ; sans script, le serveur redirige vers la page d'origine avec `?envoye=<formId>`.
