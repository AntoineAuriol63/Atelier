...
Depuis le 9 septembre 2026, la route `app/(site)/s/[sub]/[[...path]]/route.ts` est un gestionnaire de route qui renvoie le document HTML complet construit par `lib/html-document.ts` (`htmlDocument`), la même fonction que l'export : tête complète, feuille de style de la page en ligne, corps rendu par `renderToStaticMarkup`, code head et fin de body tels quels. Plus de page React ni de layout pour `/s/` : le site publié n'embarque aucun code de l'éditeur ; il porte, quand la page en a besoin, le script des formulaires et le script du site (animations et interactions, `INTERACTION_SCRIPT` du moteur de rendu), tous deux émis par le moteur, jamais dans l'éditeur. En-tête `x-atelier-version` avec la version publiée ; `cache-control: public, max-age=0, s-maxage=60, stale-while-revalidate=600` (23 septembre 2026) : le CDN garde la page une minute et la ressert jusqu'à dix minutes pendant qu'elle se régénère, donc une publication est visible au plus une minute après ; le navigateur, lui, revalide à chaque visite.
