...
### 1.4 ter Interactions et mouvements

Section Interactions de l'inspecteur (mode Design) : **apparition** à l'entrée dans l'écran (fondu, fondu en montant ou descendant, glissé, zoom, netteté ; durée, délai, courbe, rejouer à chaque passage), **« Décaler les voisins »** (la même apparition sur les éléments voisins qui suivent, +100 ms chacun : une cascade en un clic), **compteur animé** pour un texte (le nombre compte de 0 à sa valeur), **masqué au chargement**, et interactions au clic ou au survol : afficher / masquer un élément nommé de la page ou un voisin, changer la variante d'une instance. La palette ⌘K propose « Apparition · … » pour la sélection. Section Effets : **parallaxe** (vitesse relative au défilement) et, pour une boîte, **bandeau défilant** (ses enfants défilent en boucle, durée d'un tour en secondes, pause au survol). Vue de base de données en disposition Défilement : **défilement auto** toutes les N secondes. L'éditeur montre l'état d'arrivée sans jouer les animations ; l'aperçu « Voir », le site publié et l'export les jouent (respect de « réduire les animations »). Le bloc « Questions fréquentes » arrive dépliable : réponses masquées au chargement, questions qui les affichent au clic. Détail du modèle : `docs/document-model.md`, sections 4.2 et 8.
...
Sections de l'inspecteur : chacune garde l'état où on l'a laissée (mémorisé par titre, d'une session à l'autre) ; à la première rencontre, l'ouverture dépend du type sélectionné (texte : Texte et Typographie ; boîte : Disposition et Espacement ; image : Image et Dimensions). La pastille d'origine d'une valeur se lit sans la couleur (● posé ici, ◐ hérité, ◆ style partagé, ○ défaut), se focalise au clavier pour lire son explication, et se clique pour réinitialiser quand la valeur est posée ici. Un champ de longueur dont la valeur n'est pas comprise par le navigateur passe en rouge avec son explication (la valeur est gardée, sans effet). CSS brut accepte `space.4` comme `{space.4}`. Dans le tableau d'une base, le statut se lit (« Brouillon » / « Publiée », « Nouveau » / « Traité » pour les messages) et le pied rappelle qu'une entrée publiée n'est en ligne qu'à la prochaine publication ; les messages reçus sont en lecture seule.
