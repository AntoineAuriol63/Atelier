...
**Animations** (modèle § 8.4, version de schéma 3, cadrage `docs/cadrage-animation.md`) : une **animation** est une ligne de temps nommée du site, faite de **pistes** (une par élément animé : l'élément du déclencheur, ses enfants ou les cartes d'une vue, ses mots ou ses lettres, un autre élément de la page, ou un sélecteur CSS) portant des **images-clés** en millisecondes avec une courbe par segment (courbes CSS ou ressort `spring(raideur, amortissement)`, rendu en `linear(…)`) ; boucle et aller-retour ; décalage par rang quand une piste vise plusieurs éléments. Un **déclencheur** posé sur un élément (ou sur une page) la lance : au chargement, à l'entrée dans l'écran (une fois ou à chaque passage), au survol (avec retour au départ de la souris), au clic (avec bascule), au défilement (plage), à la souris (axe), avec délai et pause au survol. Rendu : chargement et survol en CSS pur, le reste par le script (API Web Animations) ; sans script ou avec « réduire les animations », l'élément reste au repos ; une animation ne pose rien dans le style. Dans l'inspecteur Design, la section Animations propose trois choix rapides sur l'élément seul, Apparition (avec « lettre par lettre » ou « enfant par enfant »), Au survol, En continu, et liste les déclencheurs avec « Jouer » et leurs réglages ; Thème → Animations liste les lignes de temps du site (renommer, usages, supprimer). Le mode Animation (lecteur, ligne de temps, édition des images-clés par les panneaux Design) est en cours de construction ; « Compteur » anime le nombre d'un texte ; la palette ⌘K garde « Apparition · … ». Les documents d'avant sont convertis à la lecture (migrations 1 → 2 → 3).
