...
| `form` | Formulaire (envois : `docs/formulaires.md`) | oui | `formId: Id`, `successMessage: Localized<string>`, `successAction`, `notifyTo` (destinataires, séparés par des virgules), `endpoint?: string` (service d'envoi externe ; vide : la route d'Atelier) |
...
type Track = {
  id: Id; target: TrackTarget; stagger?: { each: number; from?: "start" | "end" | "center" }; keyframes: Keyframe[];
  start?: { after: Id; gap?: number } | { with: Id; gap?: number };   // placée par rapport à une autre piste de la même animation (à sa fin, ou à son départ), plus `gap` ms
};
type Animation = {
  id: Id; name: string;
...
Page.triggers?: Trigger[];                   // déclencheurs de page : l'élément porteur est la racine de la page ; au défilement, la progression de toute la page (0 en haut, 1 en bas) parcourt la ligne de temps
...
- En attendant sa portée, l'élément montre la première image ; après, il reste sur la dernière (remplissage `both`, sans réglage). Exception : une piste en attente (délai du déclencheur ou départ tardif) dont **toutes** les propriétés sont déjà animées par d'autres pistes de la même cible ne montre pas sa première image en attendant (remplissage `forwards`) : sinon son état de départ écraserait les autres pendant l'attente (une pulsation en attente cachait un zoom). Une piste qui possède au moins une propriété à elle, comme l'opacité d'une apparition, garde son état de départ. Une apparition finit donc par l'état de repos, et un survol reste grossi tant que la souris est là.
- Une propriété absente d'une image-clé est interpolée entre les images-clés qui la règlent (aux extrémités de la portée, elle vaut l'état de repos). Pour revenir au neutre au milieu d'une piste, l'image-clé porte la valeur neutre explicite (`none` pour une transformation ou un filtre) : c'est ce qu'écrit l'éditeur.
- `preset` garde le préréglage d'origine. Une animation est « intacte » (`isPresetIntact`) tant qu'elle a une seule piste à cible relative (décalage et découpage libres) dont les images-clés reprennent celles du préréglage aux mêmes proportions (mise à l'échelle permise), avec les mêmes styles, courbes, répétitions et aller-retour ; sinon l'interface la dit « personnalisée ». Le champ n'est pas effacé par une retouche : annuler la retouche rend l'animation intacte.
...
- Édition (mode Animation, aides de `packages/model/src/animations.ts`) : régler une propriété à l'instant `t` d'une piste pose ou complète l'image-clé à `t` (`planSetKeyframe`) ; retirer la propriété (`planUnsetKeyframeProp`) laisse l'image-clé. La valeur montrée à `t` (`keyframeStyleAt`) est celle de l'image-clé posée à `t`, sinon de la dernière image-clé d'avant qui règle la propriété, sinon (avant la portée) de la première, sinon l'état de repos ; elle n'est pas interpolée, l'aperçu montre l'état exact. Déplacer un groupe d'images-clés (`planShiftKeyframes`, avec `duplicate` pour copier) ne les fait jamais passer avant 0 et remplace l'image-clé déjà présente à l'arrivée ; la durée de l'animation s'allonge si besoin. Changer la durée d'une animation (`planScaleAnimation`) met ses pistes à l'échelle depuis sa longueur réelle, images-clés et décalages compris ; sans piste, c'est seulement la longueur de la ligne de temps. La vitesse d'un choix rapide (`QUICK_SPEEDS` : rapide × 0,6, normale × 1, lente × 1,6 de la durée du préréglage) garde l'animation intacte. « Animer cet élément » (`planAnimateElement`) pose un déclencheur à l'entrée dans l'écran et une animation dont la seule piste vise l'élément lui-même, avec une image-clé de repos à 0 ms. Une nouvelle piste vise l'hôte en relatif (`{ trigger: true }`) ou l'élément choisi (`{ node }`), et commence par une image-clé vide à 0 ms.
- Les interactions d'état (afficher, masquer, changer de variante) restent dans `Node.interactions`. Les **effets continus** restent des propriétés, montrés dans le mode Animation : le bandeau (`props.marquee: { duration, direction: "left" | "right" | "up" | "down", pauseOnHover }`), la parallaxe (`props.parallax`), le compteur (`props.countUp`) et le défilement automatique d'une vue en carrousel (`props.view.autoplay`).
- Choix rapides (préréglages en un geste) : au plus un déclencheur par famille de préréglage (apparition, survol, continu) sur un élément (`quickAnimation`, `planQuickAnimation`, `planQuickDetail`) ; retirer un déclencheur retire son animation si plus rien ne la lance (`planRemoveTriggerWithAnimation`, `planRemovePageTriggerWithAnimation`).
- **Enchaînement** (`start`) : une piste peut être placée par rapport à une autre piste de la même animation, `after` (à sa fin : sa dernière image-clé, plus le décalage de son dernier enfant, mot ou lettre ; pour une vue sans limite, une seule carte est comptée) ou `with` (à son départ), plus `gap` ms (≥ 0, absent = 0). Ses images-clés gardent leurs écarts ; leurs temps sont recalculés après chaque modification de l'animation (`layoutTracks`) : première image-clé = ancre + écart. Une piste sans `start` garde ses temps. Une référence à une piste absente, ou un cycle, laisse la piste à ses temps. Le rendu ignore `start` : les temps des images-clés font foi. Modifier à la main le départ d'une piste enchaînée (déplacer ses images-clés, « Départ ») change son écart ; déplacer une piste et celle qui la suit du même temps ne change rien à l'écart. Retirer une piste : celles qui la suivaient prennent son `start` en gardant leur écart (ou, si elle n'en avait pas, repartent de son départ plus leur écart). Changer la durée d'une animation met aussi les écarts à l'échelle.
- **Apparitions** (`packages/model/src/appearance.ts`) : les règles, noir sur blanc.
  1. *Ce qui compte comme apparition.* Une piste d'au moins deux images-clés qui vise l'élément, dans une animation sans boucle lancée au chargement ou à l'entrée dans l'écran, et dont le préréglage n'est pas d'une autre famille (survol, continu, attention). Elle est cherchée sur l'élément lui-même d'abord (`{ trigger: true }`, préréglage d'apparition avant piste reconnue avant piste libre), sinon dans l'animation d'un autre élément de la même page ou du même composant (`{ node }`, préréglage reconnu avant piste libre). Le préréglage d'une piste est reconnu quels que soient son départ et sa longueur (`trackPresetMatch`) ; `factor` et la vitesse (rapide, normale, lente, sur mesure) en découlent.
  2. *Ce qui la fait démarrer* (`begin`) se lit dans `start` : après (`after`) ou en même temps (`with`) que l'élément de la piste référencée, délai = écart ; sans `start`, l'élément se lance lui-même (`own`, délai = délai du déclencheur + départ de la piste) ou part avec l'élément qui lance l'animation (`host`). Les éléments après lesquels on peut démarrer (`appearanceAnchors`) : ceux de la même animation par ordre de départ, puis ceux de la page dans l'ordre du document ; une animation lancée par plusieurs déclencheurs ne reçoit pas de suite.
  3. *Démarrer après ou avec un autre élément* (`planAppearanceStart`) : la piste rejoint l'animation de cet élément, rattachée par `start` ; un élément qui lançait sa propre animation l'emmène entière (sa suite reste derrière lui) ; dans la même animation, démarrer après un élément qui le suit déjà inverse l'ordre sans cycle ; revenir à « lui-même » lui rend un déclencheur (piste ramenée à 0), ou le remet au départ de l'animation qu'il lance. « Quand un bloc qui le contient entre dans l'écran » (`within`, blocs proposés par `appearanceStartOptions` : sections et blocs nommés, les trois plus proches) passe l'animation (et sa suite) à un déclencheur d'entrée dans l'écran du bloc, ou la fait rejoindre, au départ, l'apparition que le bloc lance déjà.
  4. *Délai* (`planAppearanceDelay`) : l'écart d'une piste rattachée ; pour un élément qui part en premier, son départ dans la ligne de temps, le délai du déclencheur étant replié dedans (un seul délai).
  5. *Vitesse, effet, détail* (`planAppearanceSpeed`, `planAppearancePreset`, `planAppearanceDetail`) : seule la piste de l'élément change, ce qui la suit est replacé ; « un à un » suppose un décalage réel ; poser un effet sur un élément qui lance une suite sans bouger lui-même ajoute sa piste au départ de la suite. *Rejouer* (`planAppearanceReplay`) se règle sur le déclencheur qui lance, pour toute la scène.
  6. *Héritage* (`inheritedAppearance`) : un élément sans apparition propre arrive avec la liste qui fait arriver ses enfants (la carte), ou avec le bloc qui arrive d'un seul tenant ; un préréglage choisi sur une carte s'applique à la liste.
  7. *Pareil pour ce qui suit* (`planAppearanceCascade`) : les voisins qui suivent, sans apparition propre, reçoivent une copie de la piste dans la même animation, chacun rattaché « avec » le précédent plus l'écart ; une animation lancée par plusieurs déclencheurs ne reçoit rien. *Reprendre une piste* (`planDuplicateTrack`) copie images-clés, décalage et courbes pour un autre élément, qui part après la piste d'origine.
  8. *Suppression* (`planRemoveNode`, `planForgetNodes`) : les pistes d'un élément supprimé quittent les animations des autres (ce qui les suivait se raccroche) ; une suite qu'il lançait est reprise par le premier élément restant (sa piste devient relative, le déclencheur passe sur lui) ; détacher une instance reporte ses déclencheurs et les pistes qui la visent sur la copie.
  9. *Rendu* : une instance de composant peut porter des déclencheurs et être visée par une piste ; la racine rendue de son composant porte la classe de l'instance, ses déclencheurs (`data-anim`) et sa marque de cible.

Rendu (`packages/renderer`) : chaque piste de chaque animation devient un bloc `@keyframes at-<animation>-<piste>` (positions en % de sa portée, courbe d'un segment posée en `animation-timing-function` sur l'image qui l'ouvre, ressort échantillonné en `linear(…)` sur la durée du segment). Pour chaque déclencheur, la règle est émise sur la cible résolue (`.n-hôte`, `.n-hôte>*`, `.n-hôte .at-piece`, `.n-cible`, `.at-page:has(.n-hôte:hover) .n-cible` au survol d'un autre élément) : `animation: at-… <portée>ms … <délai + début de portée>ms <répétitions> <sens> both`, avec `animation-delay` en `calc()` et `--at-i`/`--at-n` sur les éléments quand il y a décalage. `load` et `hover` sans retour sont en CSS pur ; `inView` est en pause (`animation-play-state: paused`) jusqu'au script ; `click`, `scroll` (la position de défilement parcourt la ligne de temps), `pointer` (la souris la parcourt), le retour au départ de la souris (`reverse()`), la bascule au clic, et toute piste à sélecteur libre sont joués par le script avec l'API Web Animations (mêmes courbes qu'en CSS : une par segment, `ease` par défaut, progression linéaire sur la piste ; au défilement et à la souris, sans délai de déclencheur ni répétitions, décalage selon le rang parmi toutes les cibles), qui lit `data-anim` (déclencheurs et pistes recopiés, déclarations résolues ; `pg` marque un déclencheur de page, dont le défilement suit la progression de toute la page). Les éléments animés qui ne sont pas le porteur reçoivent `data-anim-target` ; les règles « rien dans l'éditeur » (`.at-page[data-editor]`), « réduire les animations » et `<noscript>` s'appuient sur `[data-anim]` et `[data-anim-target]`. Un texte découpé se rend en `<span class="at-piece">` par mot, ou par lettre dans un `<span class="at-word">` par mot ; espaces hors des morceaux, marques conservées, `aria-label` en lettres ; un texte lié à un champ « texte long » ne se découpe pas. Dans l'éditeur, aucune animation ne joue d'elle-même : le mode Animation montre l'état à la tête de lecture, « Jouer » rejoue une animation sur ses cibles. Une animation ne pose rien dans `style` : la retirer laisse l'élément tel quel.

Identifiants : la migration donne à chaque animation un identifiant libre même quand deux runs copiés partageaient le même ; à toute lecture, des animations de même identifiant sont réparées (doubles identiques fusionnés, doubles différents renommés, les déclencheurs restant sur le premier). Une copie d'élément renouvelle les identifiants de ses déclencheurs et lance les mêmes animations.
