...
- Apparition d'un élément (`packages/model/src/appearance.ts`) : une piste « apparition » vise l'élément dans une animation lancée au chargement ou à l'entrée dans l'écran, sans boucle ; sur l'élément lui-même d'abord (`{ trigger: true }`), sinon dans l'animation d'un autre élément de la même page ou du même composant (`{ node }`). Son préréglage est reconnu quel que soit son départ et sa longueur (`trackPresetMatch`). **Ce qui la fait démarrer** (`appearanceOf(...).begin`) se lit dans `start` : après ou en même temps que l'élément de la piste référencée (délai = écart) ; sans `start`, l'élément se lance lui-même (délai = délai du déclencheur + départ de la piste) ou part avec l'élément qui lance l'animation. Réglages : démarrer après ou avec un autre élément (`planAppearanceStart` : la piste rejoint l'animation de cet élément avec `start` ; un élément qui lançait sa propre animation l'emmène entière ; démarrer après un élément qui le suit déjà inverse l'ordre sans cycle ; revenir à « lui-même » lui rend un déclencheur, ou le remet au départ de l'animation qu'il lance) ; délai (`planAppearanceDelay` : l'écart, ou pour un élément qui part en premier son départ, le délai du déclencheur étant replié dans la ligne de temps) ; vitesse, effet, détail (`planAppearanceSpeed`, `planAppearancePreset`, `planAppearanceDetail` : seule sa piste change, ce qui la suit est replacé) ; rejouer (`planAppearanceReplay`, sur le déclencheur qui lance). Poser un effet sur un élément qui lance une suite sans bouger lui-même ajoute sa piste au départ de cette suite, sans la remplacer. Démarrer « quand un bloc qui le contient entre dans l'écran » (`within`, blocs proposés par `appearanceStartOptions` : sections et blocs nommés, les plus proches) passe l'animation de l'élément (et sa suite) à un déclencheur d'entrée dans l'écran de ce bloc, ou la fait rejoindre, au départ, l'apparition que ce bloc lance déjà. Un élément sans apparition propre qui arrive avec un autre (la carte d'une liste qui fait arriver ses enfants, ou un élément dans un bloc qui arrive d'un seul tenant) se lit par `inheritedAppearance`. « Un à un » (`detail: "children"`) suppose un décalage réel. Supprimer un élément (`planRemoveNode`) retire aussi ses pistes des animations des autres éléments, et une suite qu'il lançait est reprise par le premier élément restant de cette suite ; détacher une instance reporte ses déclencheurs et les pistes qui la visent sur la copie. Rendu : une instance de composant peut porter des déclencheurs et être visée par une piste ; la racine rendue de son composant porte la classe de l'instance, ses déclencheurs (`data-anim`) et sa marque de cible.
