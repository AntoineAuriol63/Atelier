...
- Apparition d'un élément (`packages/model/src/appearance.ts`, sans champ supplémentaire) : lue depuis les pistes. Une piste « apparition » vise l'élément dans une animation lancée au chargement ou à l'entrée dans l'écran, sans boucle ; sur l'élément lui-même d'abord (`{ trigger: true }`), sinon dans l'animation d'un autre élément de la même page ou du même composant (`{ node }`). Son préréglage est reconnu quel que soit son départ et sa longueur (`trackPresetMatch`) ; sa fin compte le décalage du dernier enfant ou morceau (`trackEnd`). **Ce qui la fait démarrer** (`appearanceOf(...).begin`) se lit dans la ligne de temps : « en même temps que » une piste listée avant elle au même départ, sinon « après » la piste dont la fin est la plus proche avant son départ (le délai est l'écart), sinon l'élément se lance lui-même (délai du déclencheur compris), sinon elle part avec l'élément qui la lance. Réglages : démarrer après ou avec un autre élément (`planAppearanceStart` : la piste rejoint l'animation de cet élément, placée à sa fin ou à son départ ; un élément qui lançait sa propre animation l'emmène entière, ses pistes relatives devenant `{ node }` ; revenir à « lui-même » lui rend un déclencheur et une animation ramenée à 0) ; délai (`planAppearanceDelay` : le délai du déclencheur est replié dans la ligne de temps quand l'élément part en premier) ; vitesse, effet, détail (`planAppearanceSpeed`, `planAppearancePreset`, `planAppearanceDetail` : seule sa piste change) ; rejouer (`planAppearanceReplay`, sur le déclencheur qui lance). Quand une piste change, ce qui partait en même temps qu'elle (listé après, même départ) suit son départ et ce qui partait après sa fin suit sa fin ; retirer une piste rapproche d'autant ce qui la suivait, et une animation où plus rien ne bouge part avec ses déclencheurs. Démarrer « quand un bloc qui le contient entre dans l'écran » (`planAppearanceStart` avec `within`, blocs proposés par `appearanceStartOptions` : sections et blocs nommés, les plus proches) passe l'animation de l'élément (et sa suite) à un déclencheur de ce bloc, ou la fait rejoindre, au départ, l'apparition que ce bloc lance déjà. Un élément sans apparition propre qui arrive avec un autre (la carte d'une liste qui fait arriver ses enfants, ou un élément dans un bloc qui arrive d'un seul tenant) se lit par `inheritedAppearance`. « Un à un » (`detail: "children"`) suppose un décalage réel. Rendu : une instance de composant peut porter des déclencheurs et être visée par une piste ; la racine rendue de son composant porte la classe de l'instance, ses déclencheurs (`data-anim`) et sa marque de cible.
